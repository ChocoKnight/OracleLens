...
const GameService = {
  async getAll() {
    const [rows] = await import_mysql.default.execute("SELECT * FROM games");
    return rows;
  },
  async getByMatch(matchId) {
    const [rows] = await import_mysql.default.execute("SELECT * FROM games where match_id = ?", [matchId]);
    return rows;
  },
  async getByMatchGameNumber(matchId, gameNumber) {
    const [rows] = await import_mysql.default.execute("SELECT * FROM games WHERE match_id = ? and game_number = ?", [matchId, gameNumber]);
    const games = rows;
    return games.length > 0 ? games[0] : null;
  },
  async getOne(gameId) {
    const [rows] = await import_mysql.default.execute("SELECT * FROM games WHERE id = ?", [gameId]);
    const games = rows;
    return games.length > 0 ? games[0] : null;
  },
  async create(game) {
